...
(() => {
  ...
  var paneViewerSize = (agentCount = 1) => {
    const size = tileSize();
    ...
    return {
      ...
      height: Math.min(720, Math.max(270, Math.floor(size.height / rows2)))
    };
  };
  var rootViewerSize = () => {
    const size = tileSize();
  ...
  var withTimeout = (promise, timeoutMs, label) => new Promise((resolve, reject) => {
    const timeout = window.setTimeout(() => reject(new Error(`${label} timed out after ${Math.ceil(timeoutMs / 1e3)}s`)), timeoutMs);
    void promise.then(
      (value) => {
  ...
  var httpErrorFromResponse = async (response, label) => {
    let bodyText = "";
    try {
...
`;
  ...
  `;
    ...
    const writeLog = (target, line, direction = "sys", maxRows = 56) => {
    ...
    const formatAggregateAgentTime = (elapsedMs) => {
    ...
    const startAggregateTimeTicker = () => {
      ...
      updateAggregateTime();
      aggregateTimeTimer = window.setInterval(updateAggregateTime, 1e3);
    };
    const graphNode = (id) => {
      if (id === rootAgentId) {
        return {
    ...
    const renderGraphFor = (container, startId, compact) => {
      ...
      const nodeMetricsAtDepth = (depth) => {
      ...
      const resolveDepthCollisions = () => {
        const pointsByDepth = /* @__PURE__ */ new Map();
      ...
      const centerParentsOnChildren = (id) => {
        const point = points.get(id);
        if (point === void 0) return paddingY;
      ...
      points.forEach((_point, id) => {
        ...
        importedAgentsFor(agent).filter((imported) => points.has(imported.id) && !childIds.has(imported.id)).forEach((imported) => edges.push({ from: id, to: imported.id, kind: "import" }));
      });
      const columnXs = [];
      ...
      const nodeSvg = Array.from(points.entries()).map(([id, point]) => {
        ...
        const primaryY = secondaryLabel === "" && bomLabel === "" ? Math.round(metrics.height / 2 + Math.max(4, metrics.scale * 5)) : Math.round(metrics.height * 0.31);
    ...
    const waitForSnapshotView = async () => {
      if (!isControllerWindow) throw new Error("only the controller can render CAD snapshots");
      if (snapshotViewDisposing !== void 0) await snapshotViewDisposing;
      if (snapshotView?.rtc !== void 0) return snapshotView;
      if (snapshotViewStarting !== void 0) return snapshotViewStarting;
      snapshotViewStarting = (async () => {
        if (snapshotView !== void 0) {
        ...
        while ((!transportReady || view.rtc === void 0 || video?.srcObject === null || (video?.readyState ?? 0) < HTMLMediaElement.HAVE_CURRENT_DATA) && Date.now() < deadline) await wait(80);
    ...
    const disposeSnapshotView = async () => {
      ...
      const disposing = (async () => {
        if (previous !== void 0) {
          await withTimeout(Promise.resolve(previous.deconstructor()), snapshotDisposeTimeoutMs, "snapshot renderer shutdown").catch(() => {
          });
        }
      })();
    ...
    const resolveImportFilePath = (specifier) => {
      if (kclFiles.has(specifier)) return specifier;
      return Array.from(kclFiles.keys()).find((filePath) => renderPathForFilePath(filePath) === specifier || filePath.endsWith(`/${specifier}`));
    };
    const importFilePathsFromSource = (source) => {
      const paths = [];
      ...
      for (; ; ) {
        ...
        if (filePath !== void 0) paths.push(filePath);
      }
      return paths;
    };
    const collectFilePathsWithImports = (entryPaths) => {
    ...
    const installViewHealthWatchdog = (view, onFailure) => {
      ...
      return window.setInterval(() => {
        ...
        if (video?.error !== null && video?.error !== void 0) {
          onFailure(`video error: ${video.error.message || video.error.code}`);
          return;
        }
        if (view.rtc === void 0 || video === null || video.srcObject === null) {
        ...
        if (lastFrameCount >= 0 && !video.paused && Date.now() - lastProgressAt > viewerStalledReloadMs) {
          onFailure(`video frame stream stalled for ${Math.round((Date.now() - lastProgressAt) / 1e3)}s`);
        }
      }, viewerHealthPollMs);
    };
    function attachCenterViewHandlers(view) {
    ...
    function scheduleCenterViewerReload(reason) {
      ...
      rootLogLine(`< center renderer error; reload ${attempt}/${maxViewerReloadAttempts} in ${Math.round(delay / 1e3)}s: ${reason}`, "in");
    ...
    const scheduleRootProjectSubmit = () => {
      if (rootRenderTimer !== void 0) window.clearTimeout(rootRenderTimer);
      rootRenderTimer = window.setTimeout(() => {
        rootRenderTimer = void 0;
        submitRootProject();
      }, 450);
    };
    const liveAgentViewLimit = () => Math.min(maxLiveAgentViews, Math.max(1, plannedAgentCount || agents.size || 1));
    const liveAgentViewCount = () => Array.from(agents.values()).filter((agent) => agent.view !== void 0).length;
    const markViewerQueued = (agent, reason) => {
      agent.viewerSlot?.classList.add("agent-viewer-queued");
      if (agent.viewPlaceholder !== void 0) {
    ...
    const setAgentViewerPlaceholder = (agent, message) => {
    ...
    const clearAgentViewerTimers = (agent) => {
      ...
      if (agent.viewerHealthTimer !== void 0) {
        window.clearInterval(agent.viewerHealthTimer);
        agent.viewerHealthTimer = void 0;
      }
    };
    const disposeAgentView = async (agent) => {
    ...
    function scheduleAgentViewerReload(agent, reason) {
      ...
      appendAgentLog(agent, `< renderer error; reload ${attempt}/${maxViewerReloadAttempts} in ${Math.round(delay / 1e3)}s: ${reason}`, "in");
    ...
    function attachAgentViewHandlers(agent, view) {
      if (agent.viewerHealthTimer !== void 0) window.clearInterval(agent.viewerHealthTimer);
      ...
      view.addEventListener("status", (ev) => {
        if (!(ev instanceof CustomEvent)) return;
        const message = String(ev.detail);
        if (agent.logElement !== void 0) writeLog(agent.logElement, `rtc: ${message}`);
    ...
    const ensureAgentView = (agent, reason) => {
      if (agent.view !== void 0) {
      ...
      const viewerSlot = agent.viewerSlot;
      if (viewerSlot === void 0) return false;
      const view = new ZooWebView({
        zooClient,
    ...
    const submitAgentProject = async (agent, onSuccess) => {
      ...
      if (agent.view === void 0 && !ensureAgentView(agent, "final KCL ready")) {
        return { ok: true, message: "viewer budget queued" };
      }
      if (agent.view?.rtc === void 0) return { ok: false, message: "agent view not connected yet" };
    ...
    const submitDraftWorkAgentProject = async (agent, currentRun, draftKcl, draftIndex) => {
      ...
      if (agent.id !== rootAgentId && agent.view === void 0 && !ensureAgentView(agent, "draft KCL stream")) {
        return { ok: true, message: "viewer budget queued" };
      }
      const view = agent.id === rootAgentId ? centerView : agent.view;
      if (kclFiles.size === 0 || view === void 0) return { ok: false, message: "draft view not ready" };
      if (view.rtc === void 0) return { ok: true, message: "draft renderer starting" };
      let failureMessage = "";
      let viewerFailure = false;
      try {
    ...
    const importedAgentsFor = (agent) => (agent.imports ?? []).map((filePath) => agentForFilePath(filePath)).filter((candidate) => candidate !== void 0 && candidate.id !== agent.id);
    const importConsumersFor = (agent) => Array.from(agents.values()).filter((candidate) => candidate.kind === "orchestrator" && candidate.id !== agent.parentId && (candidate.imports ?? []).includes(agent.filePath));
    const escapeRegExp = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const kclForGraphAgent = (agent) => kclFiles.get(agent.id === rootAgentId ? rootFilePath : agent.filePath) ?? "";
    const cleanBomBody = (source) => stripImportLines(source).split("\n").map((line) => line.replace(/\/\/.*$/, "").trim()).filter(Boolean).join("\n");
    const sharedReusableImportsFor = (agent) => importedAgentsFor(agent).filter((imported) => imported.kind === "worker" && imported.scope === "shared_part");
    const explicitBomQuantityForReusable = (source, reusable) => {
      ...
      for (const line of source.split("\n")) {
        ...
        const quantity = lower.match(/\b(\d+)\s*x\b/)?.[1] ?? lower.match(/\bx\s*(\d+)\b/)?.[1] ?? lower.match(/\bquantity\s*[:=]\s*(\d+)\b/)?.[1];
        if (quantity !== void 0) return Number(quantity);
      }
      return void 0;
    };
    const quantityForReusableImport = (consumer, reusable) => {
      const source = kclForGraphAgent(consumer);
      const explicitQuantity = explicitBomQuantityForReusable(source, reusable);
      if (explicitQuantity !== void 0) return explicitQuantity;
    ...
    const mergeBomUsage = (usage, item) => {
    ...
    const directBomUsageFor = (agent) => sharedReusableImportsFor(agent).map((reusable) => ({
      agent: reusable,
      quantity: quantityForReusableImport(agent, reusable)
    }));
    const rolledUpBomUsageFor = (agent, visited = /* @__PURE__ */ new Set()) => {
      ...
      graphChildren(agent.id).filter((child) => !isReusableLibraryAgent(child)).forEach((child) => {
        rolledUpBomUsageFor(child, visited).forEach((item) => mergeBomUsage(usage, item));
      });
      return Array.from(usage.values()).sort((a, b) => b.quantity - a.quantity || a.agent.role.localeCompare(b.agent.role));
    };
    const bomSummaryForGraphNode = (agent, compact) => {
      ...
      const labels = usage.slice(0, maxItems).map((item) => `${item.quantity > 0 ? item.quantity : "?"}x ${titleCase(item.agent.role)}`);
      const remainder = usage.length - labels.length;
      return `BOM: ${labels.join(", ")}${remainder > 0 ? ` +${remainder}` : ""}`;
    };
    const placementComponentsFor = (agent) => {
      const seen = /* @__PURE__ */ new Set();
    ...
    const reviewWorkerTargets = (agent) => {
      ...
      const seen = /* @__PURE__ */ new Set();
      return [...collect(agent.id), ...importedAgentsFor(agent).filter((child) => child.kind === "worker")].filter((child) => {
    ...
    const isPlacementRework = (request) => /\b(assembly|assemble|place|placement|position|translate|rotate|align|axis convention|datum|origin|mate|layout|duplicate|stray|unassembled|integrat|import|clone|mirror|mirrored|symmetry|symmetric|pattern|array|radial|linear|spacing|pitch|bolt circle|hidden|unplaced|root|transform|coaxial)\b/i.test(`${request.target} ${request.reason} ${request.instruction}`);
    ...
    const placementInstructionFor = (parent, changedChild, extra = "") => {
      ...
      const failedChildren = failedWorkerTargets(parent).map((child) => `- ${child.role} (${child.filePath})`).join("\n");
      return [
...
${interfaces}` : "",
    ...
    const updateViewerPlaceholderText = (agent) => {
      if (agent.viewPlaceholder === void 0) return;
    ...
    const setAgentStatus = (agent, status) => {
      ...
      const isTimedStatus = (value) => value === "running" || value === "reviewing";
      if (previousStatus !== status) {
        const now = Date.now();
        if (isTimedStatus(previousStatus) && agent.activeStartedAtMs !== void 0) {
          agent.elapsedMs = (agent.elapsedMs ?? 0) + Math.max(0, now - agent.activeStartedAtMs);
          agent.activeStartedAtMs = void 0;
        }
    ...
    const syncReusableLibraryMetadata = (library) => {
      ...
      const lines = [
        ...
        ...sharedChildren.flatMap((child) => [
          ...
          `// file: ${child.filePath}`,
          `// status: ${child.status}`,
          ...interfaceManifests.has(child.filePath) ? interfaceManifests.get(child.filePath).split("\n").slice(0, 12).map((line) => `//   ${line}`) : []
        ]),
        "// /ZOOKEEPER_BOM_LIBRARY",
        ""
      ];
    ...
    const syncAssemblyFileImports = (agent) => {
    ...
    const findReusableLibraryAgent = () => Array.from(agents.values()).find((agent) => isReusableLibraryAgent(agent));
    const ensureReusableLibraryAgent = () => {
      ...
      if (!addAgent(agent)) return void 0;
      syncReusableLibraryMetadata(agent);
    ...
    const createSharedComponentAgent = (request, currentRun) => {
      ...
      if (ensuredLibrary === void 0) return void 0;
    ...
    const findConsumerAgent = (text, fallback) => {
      const candidates = Array.from(agents.values()).filter((agent) => agent.kind === "orchestrator" && agent.scope !== "shared_part" && !/\b(reusable|shared|library|catalog|standard)\b/i.test(agent.role));
      const ranked = rankAgentTarget(candidates, {
        target: text,
        instruction: text,
        reason: text
      });
      return ranked !== void 0 && ranked.score > 0 ? ranked.candidate : fallback;
    };
    const addImportToConsumer = (consumer, shared, reason = "") => {
      ...
      const imports = [.../* @__PURE__ */ new Set([...consumer.imports ?? [], shared.filePath])];
    ...
    const resolveBomImport = (reference, spawned) => {
      const normalized = reference.trim().toLowerCase();
      if (normalized.length === 0) return void 0;
      const local = spawned.get(normalized);
      if (local !== void 0) return local;
      return Array.from(agents.values()).find((candidate) => candidate.id.toLowerCase() === normalized || candidate.role.toLowerCase() === normalized || candidate.filePath.toLowerCase() === normalized || renderPathForFilePath(candidate.filePath).toLowerCase() === normalized || slugLabel(candidate.role) === slugLabel(reference));
    };
    const createSubassemblyBomChild = (parent, seed, currentRun) => {
      ...
      plannedAgentCount += 1;
      updateProjectFile(agent.filePath, "");
      return addAgent(agent) ? agent : void 0;
    };
    const startSubassemblyBomChild = (agent, currentRun) => {
    ...
    const requestSubassemblyBom = async (agent, currentRun, retryAttempt = 0) => {
      ...
      try {
        ...
        update.children.forEach((childSeed) => {
          ...
          if (child !== void 0) spawned.set(childSeed.key.trim().toLowerCase(), child);
        });
        update.children.forEach((childSeed) => {
          ;
    ...
    const runRootSupervisorSweep = () => {
      ...
      for (const agent of agents.values()) {
        ...
        if (readyChild === void 0 || now - lastReview < supervisorSummaryIntervalMs) continue;
    ...
    const startRootSupervisor = () => {
      if (!isControllerWindow || supervisorTimer !== void 0) return;
    ...
    const clearTimers = () => {
      ...
      if (graphRenderTimer !== void 0) {
        window.clearTimeout(graphRenderTimer);
        graphRenderTimer = void 0;
      }
      if (layoutTimer !== void 0) {
    ...
    const requestOrchestration = async (prompt) => {
      try {
        ...
        if (buffer.trim().length > 0) handleEvent(JSON.parse(buffer));
        if (plan === void 0) throw new Error("orchestrate stream ended without a plan");
        return plan;
      } catch (error) {
    ...
    const performAgentWork = async (agent, currentRun, workRevision, renderError = "", repairAttempt = 0, reviewInstruction = "", zooRetryAttempt = 0) => {
      ...
      try {
        ...
        const draftText = update.drafts === void 0 || update.drafts === 0 ? "" : ` after ${update.drafts} draft${update.drafts === 1 ? "" : "s"}`;
    ...
    const runZookeeper = async () => {
      ...
      after(450 + seeds.length * 120 + 1350, () => {
        if (currentRun !== runId) return;
        const initialAgents = seeds.map((seed) => agents.get(seed.id)).filter((agent) => agent !== void 0);
    ...
    const handleWallBroadcast = (message) => {
      ...
      const agentId = "agentId" in message ? message.agentId : void 0;
      const agent = agentId === void 0 ? void 0 : agents.get(agentId);
      if (agent === void 0) return;
      ...
      if (message.type === "agent:final") {
        ...
        if (message.manifest !== void 0) interfaceManifests.set(agent.filePath, message.manifest);
        void submitAgentProject(agent);
        return;
      }
